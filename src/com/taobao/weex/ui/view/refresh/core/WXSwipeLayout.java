...
public class WXSwipeLayout extends FrameLayout implements NestedScrollingParent {
	// private String TAG = WXSwipeLayout.class.getSimpleName();
	...
	// Drag Action
	private int mCurrentAction = INVALID;
	...
	private int autoRefreshViewHeight = 75;
	...
	private void initAttrs(Context context, AttributeSet attrs) {
		...
		autoRefreshViewHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, autoRefreshViewHeight, getContext().getResources().getDisplayMetrics());
	}
		...
		postDelayed(new Runnable() {
			...
			public void run() {
				if (!mRefreshing) {
					...
					startRefresh(autoRefreshViewHeight);
				}
			}
		}, 50);
	}
	...
	public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
		...
		// Log.i(TAG, "dy--->" + dy + "--mPullRefreshEnable-->" + mPullRefreshEnable + "--mPullLoadEnable-->" + mPullLoadEnable);
		if (!isConfirm) {
		...
		// Log.i(TAG, "dy--->" + dy + "--mCurrentAction-->" + mCurrentAction);
		if (moveSpinner(-dy)) {
			consumed[1] += dy;
		}
	}
