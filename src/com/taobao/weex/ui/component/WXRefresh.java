...
public class WXRefresh extends WXBaseRefresh implements WXSwipeLayout.WXOnRefreshListener {
	...
	public void destroy() {
		...
		if (getParent() instanceof WXListComponent || getParent() instanceof WXScroller) {
			if (((BaseBounceView) getParent().getHostView()).getSwipeLayout().isRefreshing()) {
				((BaseBounceView) getParent().getHostView()).getSwipeLayout().finishPullRefresh();
				((BaseBounceView) getParent().getHostView()).getSwipeLayout().finishPullLoad();
			}
		}
	}
	...
	protected boolean setProperty(String key, Object param) {
		...
		case Constants.Name.REFRESHING:
			boolean refreshing = WXUtils.getBoolean(param, false);
			if (refreshing) {
	...
	public void setRefreshing(boolean refreshing) {
		if (getParent() instanceof WXListComponent || getParent() instanceof WXScroller) {
			if (!((BaseBounceView) getParent().getHostView()).getSwipeLayout().isRefreshing() && refreshing) {
				((BaseBounceView) getParent().getHostView()).getSwipeLayout().setRefreshing();
			}
		}
	}
}
