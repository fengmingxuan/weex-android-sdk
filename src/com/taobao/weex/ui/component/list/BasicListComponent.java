...
public abstract class BasicListComponent<T extends ViewGroup & ListComponentView> extends WXVContainer<T> implements IRecyclerAdapterListener<ListBaseViewHolder>, IOnLoadMoreListener, Scrollable {
	...
	protected boolean setProperty(String key, Object param) {
		...
		case Constants.Name.SCROLLABLE:
			...
			return true;
		case Constants.Name.SCROLL_TO_POSITION:
			if (getHostView() != null && getHostView().getInnerView() != null) {
				int position = WXUtils.getInt(param);
	...
	public void onLoadMore(int offScreenY) {
		try {
			...
			if (offScreenY < Integer.parseInt(offset)) {
				...
				if (mListCellCount != mChildren.size() || mLoadMoreRetry == null || !mLoadMoreRetry.equals(loadMoreRetry)) {
